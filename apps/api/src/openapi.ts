...
export const OPENAPI_SCHEMAS: OpenApiSchemas = {
  ...
  OpsTask: {
    ...
    properties: {
      ...
      escalationLevel: { type: 'integer', minimum: 0 },
      status: {
        type: 'string',
        enum: ['OPEN', 'CLAIMED', 'RESOLVED', 'CANCELLED'],
      },
  ...
  OpsTaskListResponse: {
    type: 'object',
    additionalProperties: false,
    required: ['actor', 'tasks', 'generatedAt'],
    properties: {
      actor: {
        type: 'object',
        additionalProperties: false,
        required: ['userId', 'roles', 'queues'],
        properties: {
          userId: { type: 'string', format: 'uuid' },
      ...
      tasks: {
        type: 'array',
        items: { $ref: '#/components/schemas/OpsTask' },
      },
  ...
  OpsTaskCommand: {
    ...
    properties: {
      ...
      reasonCode: { type: 'string', pattern: '^[a-z][a-z0-9_.-]{2,100}$' },
    },
  },
  ReassignOpsTaskRequest: {
    type: 'object',
    additionalProperties: false,
    required: ['expectedVersion', 'reasonCode', 'newOwnerUserId'],
    properties: {
      expectedVersion: { type: 'integer', minimum: 1 },
  ...
  ResolveOpsTaskRequest: {
    type: 'object',
    additionalProperties: false,
    required: ['expectedVersion', 'reasonCode', 'resolutionCode'],
    properties: {
      expectedVersion: { type: 'integer', minimum: 1 },
      reasonCode: { type: 'string' },
  ...
  NotificationIntent: {
    ...
    properties: {
      ...
      bodyRedacted: { type: 'string' },
      correlationId: { type: 'string' },
      status: {
        type: 'string',
        enum: ['PENDING', 'LEASED', 'DELIVERED', 'TERMINAL_FAILED', 'CANCELLED'],
      },
      ...
      opsTaskId: { type: 'string', format: 'uuid', nullable: true },
      version: { type: 'integer', minimum: 1 },
      createdAt: { type: 'string', format: 'date-time' },
      updatedAt: { type: 'string', format: 'date-time' },
    },
  ...
  NotificationAttempt: {
    ...
    properties: {
      ...
      status: {
        type: 'string',
        enum: ['FIRED', 'FAILED', 'DEAD_LETTER'],
      },
      providerMessageRef: { type: 'string', nullable: true },
  ...
  NotificationIntentListResponse: {
    type: 'object',
    additionalProperties: false,
    required: ['intents', 'generatedAt'],
    properties: {
      intents: {
        type: 'array',
        items: { $ref: '#/components/schemas/NotificationIntent' },
      },
      generatedAt: { type: 'string', format: 'date-time' },
    },
  },
  NotificationAttemptListResponse: {
    type: 'object',
    additionalProperties: false,
    required: ['intentId', 'attempts', 'generatedAt'],
    properties: {
      intentId: { type: 'string', format: 'uuid' },
      attempts: {
        type: 'array',
        items: { $ref: '#/components/schemas/NotificationAttempt' },
      },
      generatedAt: { type: 'string', format: 'date-time' },
    },
  },
